...
var initIndexedDb = () => {
	var db = new Dexie("qmethod");
	db.version(1).stores({
		statements: 'id, statement',
		statements_category: 'id, statement, category'
	});
	return db;
}

var saveStatementCategory = (item) => {
	var db = initIndexedDb();	

	db.statements_category.put({
		id: item.id,
...
var saveStatements = (items) => {
	var db = initIndexedDb();	

	for(var i = 0; i <= items.length; i++) {
		db.statements.put({
			id: items[i].id,
			statement: items[i].statement
		});
	}	
}

var checkContinue = async () => {
	var db = initIndexedDb();
	return await db.statements.count() > 0;
}
...
var getStatementsAgree = () => {
	var db = initIndexedDb();
	return db.statements_category.where('category').equalsIgnoreCase('agree').toArray();
}

var getStatementsNeutral = () => {
	var db = initIndexedDb();
	return db.statements_category.where('category').equalsIgnoreCase('neutral').toArray();
}

var getStatementsDisagree = () => {
	var db = initIndexedDb();
	return db.statements_category.where('category').equalsIgnoreCase('disagree').toArray();
}
...
app.controller("step3Ctrl", function ($scope, $rootScope, $state) {
	getStatements().then((statements) => {
		if(statements.length > 0) {
			$rootScope.statements = statements;
		} else if (typeof $rootScope.statements == "undefined") {
			statements = [
	...
	getStatementsAgree().then((agree) => {
		getStatementsNeutral().then((neutral) => {
			getStatementsDisagree().then((disagree) => {
				$scope.classifications = {
					'AGREE': agree,
					'NEUTRAL': neutral,
					'DISAGREE': disagree,
				};
				$scope.cards.first = agree.length + neutral.length + disagree.length
				$rootScope.classifications = $scope.classifications;
				$scope.$apply();
			});
		});
